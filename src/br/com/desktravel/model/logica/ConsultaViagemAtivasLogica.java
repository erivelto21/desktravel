...
public class ConsultaViagemAtivasLogica implements Logica{
	...
	public void executa(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse)
		...
		if(usuario.getTipo().getId() == 2){
			ArrayList<Viagem> vv = dao.ativa(usuario.getId());
			...
			ArrayList<Viagem> vv = dao.ativaAdm();
			httpServletRequest.setAttribute("viagens", vv);
			RequestDispatcher rd = httpServletRequest.getRequestDispatcher("privado/consulta/viagemAtivaConsultaA.jsp");
			rd.forward(httpServletRequest, httpServletResponse);
		}
	}

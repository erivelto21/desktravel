...
public class ConsultaSubordinadosLogica implements Logica{
	...
	public void executa(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse)
		...
		if(usuario.getTipo().getId() == 2){
			...
			RequestDispatcher rd = httpServletRequest.getRequestDispatcher("privado/consulta/subordinadosConsultaG.jsp");
			rd.forward(httpServletRequest, httpServletResponse);
		}else if(usuario.getTipo().getId() == 1){
			httpServletRequest.setAttribute("items", dao.subordinadosA());
			RequestDispatcher rd = httpServletRequest.getRequestDispatcher("privado/consulta/subordinadosConsultaA.jsp");
			rd.forward(httpServletRequest, httpServletResponse);
		}
